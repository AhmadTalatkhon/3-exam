...
//8
let array_delete = [2, 7, 42, 23, 6, 53, 12, 4, 18];
let array_ochirish = document.querySelector('.array_delete');
let btn_dalete = document.getElementById('btn_delete');
let och_array = document.getElementById('open_array');
btn_dalete.addEventListener('click', () => {
    array_ochirish.value
    let p = document.createElement('p')
    for (let i = 0; i < array_delete.length; i++) {
        if (array_ochirish.value == array_delete[i]) {
            delete array_dalete[i]
        }
    }
    console.log(array_delete);
    p.innerHTML = `[${array_delete.join(' ')}]`;
